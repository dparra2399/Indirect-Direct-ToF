...
if __name__ == '__main__':
    ...
    decays = [80, 400, 600, 800, 1000]
    if peak_factor is not None:
        decays = [80, 400, 600, 800, 1000]
    amps = [0.05, 0.25, 0.5, 0.75, 1]
    ...
    error_all = np.zeros((len(decays), len(amps), len(K_vals), 4))
    ...
    fig, ax = plt.subplots(len(K_vals), len(params['imaging_schemes']), figsize=(8, 10))
    ...
    for p in range(len(K_vals)):
        ...
        for i in range(len(params['imaging_schemes'])):
            ...
            im = ax[p, i].imshow(error_all[:, :, p, i], vmin=3, vmax=100)
    ...
    if peak_factor is not None:
        fig.savefig(f'dense_indirect_reflections_peakk{peak_name}_fig.svg', bbox_inches='tight', dpi=3000)
    else:
        fig.savefig(f'dense_indirect_reflections_bandlimitedk{K}_fig.svg', bbox_inches='tight', dpi=3000)
