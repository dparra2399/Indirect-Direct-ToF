...
file = np.load('data/results/ntbins_1024_monte_3000_lvls_20_exp_tmp.npz', allow_pickle=True)
...
noise_levels = ambient_levels

surf = ax.plot_surface(np.log10(noise_levels), np.log10(photons_levels), mae_idtof, color='yellow', antialiased=False, label='ID-ToF')
surf2 = ax.plot_surface(np.log10(noise_levels), np.log10(photons_levels), mae_itof, color='blue', antialiased=False, label='I-ToF')
surf3 = ax.plot_surface(np.log10(noise_levels), np.log10(photons_levels), mae_dtof_argmax, color='green',antialiased=False, label='D-ToF-ARGMAX')
...
ax.set_xlabel('log10 ambient levels')
...
surfdiff1 = ax2.plot_surface(np.log10(noise_levels), np.log10(photons_levels), diff1, color='orange',linewidth=0, antialiased=True)

ax2.set_title('difference between direct - coding')
ax2.set_xlabel('log10 ambient levels')
...
surfdiff2 = ax3.plot_surface(np.log10(noise_levels), np.log10(photons_levels), diff2, color='red',linewidth=0, antialiased=True)

ax3.set_title('difference between combined - coding')
ax3.set_xlabel('log10 ambient levels')
