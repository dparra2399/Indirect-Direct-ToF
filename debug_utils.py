...
def debbug_cw(params, ModFs, Incident, NormMeasures, norm_measurements_idtof,  norm_measurements_itof,
    ...
    for j in range(0, depths.shape[0]):
        ...
        if depths.shape[0] > 1:
            ...
            axis[0, j].axvline(x=depths[j])
            axis[0, j].axvline(x=np.argmax(id), color='green')
            axis[0, j].axvline(x=np.argmax(ind), color='pink')
        else:
            axis[0].plot(NormMeasures)
            axis[0].axvline(x=depths[j])
            axis[0].axvline(x=np.argmax(id), color='green')
            axis[0].axvline(x=np.argmax(ind), color='pink')
        ...
        if depths.shape[0] > 1:
            ...
            axis[1, j].plot(noise_id_wave, label='Decoded ID Wave')
            axis[1, j].plot(noise_ind_wave, label='Decoded Ind Wave')
            axis[1, j].plot(emitted_wave, label='Emitted Wave')
            axis[1, j].legend()
        else:
            axis[1].plot(true_wave, label='GT Wave')
            axis[1].plot(noise_id_wave, label='Decoded ID Wave')
            axis[1].plot(noise_ind_wave, label='Decoded Ind Wave')
...
def debug_direct(params, c_vals_pp, pulses_pp, decoded_depths_dtof_maxguass_pp,
    ...
    for i in range(0, depths.shape[0]):
        ...
        if depths.shape[0] > 1:
            ...
            axis[0, i].plot(clean_pp, linewidth=2, label='true depth')
            est_tshifts_pp = direct_tof_utils.depth2time(decoded_depths_dtof_maxguass_pp[:, i])
        else:
            clean_pp = np.transpose(np.squeeze(clean_pp[i]))
            axis[0].plot(clean_pp, linewidth=2, label='true depth')
            est_tshifts_pp = direct_tof_utils.depth2time(decoded_depths_dtof_maxguass_pp[i])
        ...
        if depths.shape[0] > 1:
            axis[0, i].plot(np.transpose(np.squeeze(est_pp)), label='estimated depth')
            axis[0, i].set_title('Direct Peak Power')
            axis[0, i].legend()
        else:
            axis[0].plot(np.transpose(np.squeeze(est_pp)), label='estimated depth')
            axis[0].set_title('Direct Peak Power')
            axis[0].legend()
        ...
        if depths.shape[0] > 1:
            axis[1, i].plot(np.transpose(np.squeeze(est_ave)), label='estimated depth')
            axis[1, i].set_title('Direct Average Power')
            axis[1, i].legend()
        else:
            axis[1].plot(np.transpose(np.squeeze(est_ave)), label='estimated depth')
            axis[1].set_title('Direct Average Power')
            axis[1].legend()